...
- For implementation changes, use `develop-with-tdd-guardrails` when available, preserve money invariants, and make small reviewable changes. Use real PostgreSQL tests for transaction/SQL guarantees. The checks and evidence limits are in [verification](docs/memory/verification.md).
