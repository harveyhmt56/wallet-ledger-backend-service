# Archived planning snapshot (2026-09-07)

Historical reference only; do not load by default. The original MEMORY.md is preserved verbatim below.
Its status, authorization language, proposed design and pending choices describe the planning session,
not the current repository. Read [current memory](../../../MEMORY.md) for verified state and routing.
