...
CI ([verify.yml](../../.github/workflows/verify.yml)) runs `clean verify -Pmutation` on Ubuntu 24.04 / Java 21 with Docker and keeps Surefire/Failsafe/PIT reports 14 days. The normal PIT gate targets `*.domain.*`, `TransferReceipt`, `RequiredJwtConfiguration`, `ApiProblems`, `RateLimitFilter`, `RedisRateLimiter`; the broader all-class score is a scratch measurement, not the gate ([how to reproduce](../test-gap-evidence-2026-09-13.md#reproduction-and-limits)).
