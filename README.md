...
Connections use a five-second lock timeout and fifteen-second statement timeout. No network call runs inside money transactions. See [PostgreSQL explicit locks](https://www.postgresql.org/docs/17/explicit-locking.html), [deferred constraint triggers](https://www.postgresql.org/docs/17/sql-createtrigger.html), and [Spring nested transaction semantics](https://docs.spring.io/spring-framework/reference/6.2/data-access/transaction/declarative/tx-propagation.html).
...
PIT targets the money, daily reward and sequence-projection domain policies with 80% mutation and coverage gates. Reports must contain evaluated mutants; zero-mutant and invalid runs are failures. SQL constraints are verified directly because JVM mutation does not mutate PostgreSQL triggers. See [PIT's Maven configuration](https://pitest.org/quickstart/maven/). Recorded results and measurement limits are in [build evidence](docs/build-evidence.md).
